...
export default function FloatingWhatsApp({
  ...
  return (
    <div
      className={`fixed z-50 ${positions[position] ?? positions.br} transition-opacity duration-300 ${
    ...
    >
      <a
        ...
        aria-label={label}
        className={`group relative block rounded-full bg-green-500 text-white shadow-lg hover:shadow-xl active:scale-95 transition-all focus:outline-none focus:ring-2 focus:ring-green-300 ${className}`}
      ...
      >
        ...
        <span
          className={`pointer-events-none absolute top-1/2 -translate-y-1/2 hidden md:block
            ${tipPos} ${tipAnim}
            opacity-0 group-hover:md:opacity-100 transition-all duration-200
